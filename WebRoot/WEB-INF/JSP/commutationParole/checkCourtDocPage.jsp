		                	...
		                	<td style="width:80%;">
		                	  ...
		                	  <c:if test="${modeledit eq 'yes'}">
		                	   <a class="mini-button" iconCls="icon-edit"  plain="true" onclick="editSuggestTemplate()">模板编辑</a>
		                        ...
		                        <!--  --><a class="mini-button" id="gb" iconCls="icon-close" style="display:  none;" plain="true" onclick="onCanle()">关闭</a>
		                	</td>
    ...
    <script type="text/javascript">
        ...
        function saveBigData() {
            ...
            $.ajax({
                ...
                url:"<%=path%>/saveCheckCaseSuggest.action?1=1",
                 type: "post",
                 data:{bigdata:data.annexcontent, crimid:crimid, 
	                   flowdraftid:flowdraftid, flowid:flowid, 
	                   tempid:tempid},
        ...
        function SetData() {
	        ...
	        if(menuid=="12513") {//如果菜单id是建议书管理，显示关闭、下一个按钮
	         	$("#xyg").show();
	         	$("#gb").show();
	        }
       		$.ajax({
               		 //url: "getSingleSuggestDocumentData.action",
	                 url: "getJianChaYuanopinions.action",
	                 type: "post",
	                 data:{tempid:tempid, crimid:crimid, flowdraftid:flowdraftid, flowid:flowid,
                           flowdefid:flowdefid, nowpunishmentyear:nowpunishmentyear,
                           doctype:doctype, paroletype:paroletype},
	                 success: function (text) {
	                 	if(text){
	                 		var o = mini.decode(text);
	                 		var casenumber = o.caseNumber;
                     		form.setData(o);
	                 	}
	                 }
	         });
		...
		function preview(){
        	...
        	var returnValue = '';
       		$.ajax({
	                 ...
	                 type: "post",
	                 async: false,
	                 data:{crimid:crimid, flowdraftid:flowdraftid, flowid:flowid,tempid:tempid},
	                 success: function (text) {
	        ...
	        //menuid为用于查询方案的id
        	var url = "toPreviewSuggestReport.action?1=1&menuid="+menuid+"&flowdraftid="+flowdraftid+"&tempid="+tempid+"&crimid="+crimid+"&nowpunishmenttype="+nowpunishmenttype;
        	...
        	*/
        	var title="意见书";
			 mini.open({
                    ...
                    onload: function () {
                        var iframe = this.getIFrameEl();
                        var data = {menuid : menuid, flowdraftid:flowdraftid, flowid:flowid, tempid:tempid,nowpunishmenttype:nowpunishmenttype};
                        iframe.contentWindow.SetData(data);
                    },
                    ondestroy: function (action) {
                    }
      ...
      function saveNext(){
        ...
        if(index>0){
            ...
            var url = "toSuggestionDocPage.action?1=1&menuid=12513&resid=12498&doctype=jailcommute&doctype=jailcommute&id="+id+"&flowdefid=other_zfjyjxjssp";
     ...
     //关闭返回到建议书管理列表页
     function onCanle(){
  	    url = "toSuggestionManageListPage.page?1=1";
      	self.location.href=url;
     }
    </script>
</body>
