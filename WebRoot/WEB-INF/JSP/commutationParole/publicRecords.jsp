...
<body onload="init('${menuid}');">   
	...
	<div size="38px;" showCollapseButton="false">
		...
		<td style="width:100%;">
    		...
    		<a class="mini-button"  style="display:none;" id="12593" iconCls="icon-save" plain="true" onclick="savedate()">存盘</a>
    		<a class="mini-button"  style="display:none;" id="12237" iconCls="icon-save" plain="true" onclick="savedate()">存盘</a>
    		<a class="mini-button" id="back"  iconCls="icon-close" plain="true" onclick="Close();">关闭</a>
	    	<span class="separator"></span> 
    ...
    <script type="text/javascript">
        ...
        function savedate() {
			...
			if(!startdate || !enddate) {
       		...
       		var url="<%=path%>/jxjs/savePublicRecords.json?1=1";
        	if(!docid){//新增
	            $.ajax({
	                url:encodeURI(encodeURI(url)),
	                data: {content:content,orgid:orgid,tempid:tempid,noteinfo:noteinfo},
	                type: "post",
	            ...
	            $.ajax({
	                url: encodeURI(encodeURI(url)),
	                data: {content:content,orgid:orgid,docid:docid,noteinfo:noteinfo},
	                type: "post",
	                success: function (text) {
	                	alert("操作成功!");
	                },
        ...
        function Close(){
        	...
        	var url="<%=path%>/jxjs/toCrimChoose.page?1=1&menuid="+fatherMenuid+"&tempid="+tempid;
