...
<body>
    ...
    <script type="text/javascript">
        ...
        function SetData(data) { 
        	...
        	var butMenuid = data.butMenuid;
        	var tempid = data.tempid;
        	mini.get("tempid").setValue(tempid);
        	var flowid = data.flowid;
        	mini.get("flowid").setValue(flowid);
        	var batchid = data.batchid;
        	mini.get("batchid").setValue(batchid);
       		$.ajax({
	                 url: "obtainMeetingRecord.action?1=1",
	                 type: "post",
	                 data: {crimids:crimids, butMenuid:butMenuid, tempid:tempid},
	                 success: function (text) {
	                 	if(text){
	                 		var obj = eval(text);
	                 		var data = obj[0];
		                 	...
		                 	mini.get("status").setValue(data.status);
		                 	mini.get("otherid").setValue(data.otherid);
	                 	}
	                 }
	        });
        }
