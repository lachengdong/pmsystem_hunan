...
<body onload="init('${menuid }');">
     <div class="mini-toolbar"  style="padding:2px;border-bottom: 0px;">
     	<jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
   	 	<input id="tempid" name="tempid" class="mini-hidden" value="BWJY_JBRFQJBSQS"/>
        <input id="crimid" class="mini-hidden" value="${crimid}"/>
        <input id="name" name="name" class="mini-hidden" value="${name}"/>
          <table >
             <tr>
              <td style="width:100%;">
              	<a class="mini-button" style="display:none;" id="13785" iconCls="icon-add" plain="true" onclick="add('13785')">新增</a>
              	<a class="mini-button" style="display:none;" id="13787" iconCls="icon-remove" plain="true" onclick="saveNext()">下一个</a>
    ...
    <div class="mini-fit">
	    ...
	    allowResize="false" url="<%=path%>/jubao/getSingleCrimid.json?1=1&tempid=BWJY_JBRFQJBSQS&crimid=${crimid}"  
    ...
    <script type="text/javascript"> 
     ...
     function add(menuid){
        ...
        var tempid=mini.get("tempid").getValue();
  		mini.open({
  	        url: "jubao/addjubao.page?1=1&tempid="+tempid+"&crimid="+crimid+"&menuid="+menuid,
  	        showMaxButton: true,
  	        allowResize: false,
  	        title: "放弃具保申请书", width: 900, height: 500,
  	        onload: function () {
   ...
   //删除
     function shanchu(){
         ...
         if (rows.length > 0) {
             if (confirm("确定删除选中记录？")) {
                 ...
                 var id = ids.join(',');
                 $.ajax({
                     url: "<%=path%>/jubao/deleteSinogCrimid.action?1=1&docid="+id,
                     type: "post",
 	...
 	function xiugai(menuid) {
 		...
 		var crimid = mini.get("crimid").getValue();
     	mini.open({
             url: "<%=path%>/jubao/editSinogCrimid.page?1=1&docid="+row.docid+"&menuid="+menuid,
             showMaxButton: true,
          	 allowResize: false, 
             title: "放弃具保申请书", width: 900, height: 500,
             onload: function () {
 	...
 	function chakan(menuid) {
     	...
     	mini.open({
             ...
             showMaxButton: true,
          	 allowResize: false, 
             title: "放弃具保申请", width: 900, height: 500,
             onload: function () {
                 var iframe = this.getIFrameEl();
                 var data = { action:"edit" };
                 iframe.contentWindow.SetData(data);
 	...
 	function saveNext (){
		 	var url="<%=path%>/jubao/chuliShenQing.json?1=1";
		 	var tempid=mini.get("tempid").getValue();
 	 ...
 	 //关闭返回到罪犯处理页面
    function close(){
        url = "<%=path%>/jubao/toCrimidChoosePage.action?1=1";
        self.location.href=url;
    }
</script>
</body>
