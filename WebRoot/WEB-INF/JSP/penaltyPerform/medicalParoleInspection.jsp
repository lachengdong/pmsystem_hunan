...
<body onload="init('10217_01');">
      <div class="mini-toolbar"  style="padding:2px;border-bottom: 0px;">
      <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
			  <input name="tempid" id="tempid" class="mini-hidden" value=""/>
           ...
           <table >
              <tr>
               	 <td style="width:100%;">
              		 <a class="mini-button" id="11335" iconCls="icon-add" plain="true" onclick="add('11335')">新增</a>
     ...
     <div class="mini-fit" >
	    <div id="datagrid" allowMoveColumn="false" onrowdblclick="rowdblclickfunction" style="width:100%;height:100%;" class="mini-datagrid" sizeList="[10,20,50,100]" pageSize="20"
	     	 allowAlternating="true" showLoading="true" allowResize="false" url="getMedicalParoleInspectionByCrimid.action?1=1&tempid=BWJY_JYTQZYJWZXKC&crimid=${crimid }" multiSelect="true" >
...
<script type="text/javascript">
       ...
       function onActionRenderer() {
	    ...
	    function add(menuid){
	    	...
	    	mini.open({
                url: "medicalParoleInspectionAdd.action?1=1&tempid="+tempid+"&crimid="+crimid+"&name="+name+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
                title: "保外就医考察表", width: 900, height: 500,
                onload: function () {
		...
		function chakan() {
			...
			var name=mini.get("name").getValue();
	    	mini.open({
                url: "medicalParoleInspectionAdd.action?1=1&tempid="+tempid+"&crimid="+crimid+"&name="+name+"&menuid=10217_02"+"&docid="+row.docid,
                showMaxButton: true,
             	allowResize: false, 
                title: "保外就医考察表", width: 900, height: 500,
                onload: function () {
		...
		function xiugai(menuid) {
		    ...
		    var name=mini.get("name").getValue();
	    	mini.open({
	    		url: "medicalParoleInspectionAdd.action?1=1&tempid="+tempid+"&crimid="+crimid+"&name="+name+"&menuid="+menuid+"&docid="+row.docid,
                showMaxButton: true,
             	allowResize: false, 
                title: "保外就医考察表", width: 900, height: 500,
                onload: function () {
        ...
        function shanchu(){
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        url: "medicalParoleInspectionDelete.action?1=1&docid="+id,
                        type: "post",
