...
<body onload="init('${menuid}');">
			...
			<table>
				<tr>
					<td style="width:100%;">
				    	...
				    	<a class="mini-button" id="11096" iconCls="icon-save" plain="true" onclick="savedata('','save','doc_zfrjdjsp','SDXF_BASE_RJDJB')">保存</a>
    ...
    <script type="text/javascript">
      ...
      function savedata(menuid,type,flowdefid,tempid) {
    	  ...
    	  var stringValue= aipObj.GetValueEx("cbiidentitycard",2,"",0,"");
     		var crimid = mini.get("crimid").getValue();
     		
     		var selectid = mini.get("selectid").getValue();
     		var noteinfo = getNoteMap();
     		var url="<%=path%>/property/addCrimInfol.json?1=1";
            $.ajax({
               ...
               success: function (text) {
            	   var obj = eval('(' + text + ')');
             	   if( obj.msg =='success'){
               	   		...
               	   		document.getElementById("applyid").value = obj.applyid;
               	   		document.getElementById("applyname").value = obj.applyname;
               	   		
               	   		doSubmitFlow(menuid,type,flowdefid,tempid);//保存大字段
               	   		
               	   		onCanle();
               	   }else if(obj.msg=='fysuccess'){
               	   	    alert("操作成功！");
               	   }else if(obj.msg=='0'){
               		   alert("罪犯编号重复！");
               	   }else{
       ...
       function onCanle() {
       		...
       		//跳转到信息补录列表页
        	self.location.href="<%=path%>/property/propertyPunishmentCriminalChoose.page?1=1&flowdefid=doc_zfrjdjsp";
       }
