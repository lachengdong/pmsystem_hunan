...
<body onload="init('10189');">
    <div class="mini-toolbar"  style="padding:2px;border-bottom: 0px;">
	    <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
	    <input id="tempid" name="tempid" class="mini-hidden" value="SJGL_ZFBYSJTZS"/>
         <table >
            <tr>
               <td style="width:100%;">
             	    <a class="mini-button" style="display:none;" style="display:none;" id="10189_01" iconCls="icon-add" plain="true" onclick="add('10189_01')">新增</a>
                    <!-- <a class="mini-button" style="display:none;" id="10189_02" iconCls="icon-remove" plain="true" onclick="shanchu()">批量删除</a>-->
            	</td>
    ...
    <div class="mini-fit">
	    <div id="datagrid" allowMoveColumn="false" onrowdblclick="rowdblclickfunction" style="width:100%;height:100%;" class="mini-datagrid" 
	     allowResize="false" url="<%=path%>/notInforming/ajaxGetNotInforming.json?1=1&tempid=SJGL_ZFBYSJTZS"  idField="" multiSelect="true" 
	     allowAlternating="true" sizeList="[10,20,50,100]" pageSize="20"  showLoading="true">
    ...
    function add(menuid){
		...
		mini.open({
	        url: "notInforming/notInformingAdd.page?1=1&tempid="+tempid+"&menuid="+menuid,
	        showMaxButton: true,
	        allowResize: false,
	        title: "不予收监通知", width: 900, height: 500,
	        onload: function () {
	...
	function xiugai(type) {
		var menuid = '10189_04';//默认修改
		if(type==1) menuid='10189_03';
		var row = grid.getSelected();//获取选中记录
	   	mini.open({
             url: "notInforming/notInformingEdit.page?1=1&flowdraftid="+row.flowdraftid+"&menuid="+menuid,
             showMaxButton: true,
          	 allowResize: false, 
             title: "不予收监通知", width: 900, height: 500,
             onload: function () {
    ...
    function shanchu(){
        ...
        if (rows.length > 0) {
            if (confirm("确定删除选中记录？")) {
                ...
                $.ajax({
                    url: "notInforming/deleteNotInforming.page.json?1=1&docid="+id,
                    type: "post",
