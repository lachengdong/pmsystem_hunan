...
<body onload="init('10197');">
    <div class="mini-toolbar"  style="padding:0px;border-bottom: 0px;">
    <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>   
    <input id="tempid" name="tempid" class="mini-hidden" value="SJGL_ZFSTJCB"/>
        <table >
             <tr>
                <td style="width:100%;">
                	<a class="mini-button" style="display:none;" id="11065" plain="true" onclick="chooseAll('11066')">批量处理</a>
                	<a class="mini-button" style="display:none;" id="601941" iconCls="" plain="true" onclick="chooseReview('11066')">批量重审</a>
                </td>
    ...
    <div class="mini-fit" id="div_two">
        <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" allowResize="false"  sizeList="[10,20,50,100]" pageSize="20"
          url="<%=path%>/Examination/getExaminationBodyBasicInfoList.json?1=1" idField="spid" multiSelect="true"  allowAlternating="true"  virtualScroll="false"
    ...
    <script type="text/javascript">
        	...
        	if(record.idnumber ){
	       		if(nodeids==lastnodeid){
	       			if(record.orgcontact=='1')
	       				s += ' <a class="Edit_Button"  href="javascript:chooseOne(11066);" >修改</a>';
	       			else
	       				s += ' <a class="Edit_Button"  href="javascript:chooseOne(11066);" >处理</a>';
	       		}else {
	       			s += ' <a class="Edit_Button"  href="javascript:chooseOne(11067);" >查看</a>';
	       		}
	       	}else{
	       		if(nodeids && nodeids!='0') s+='未上报';
	       		else s += ' <a class="Edit_Button"  href="javascript:chooseOne(11066);" >处理</a>';
	       	} 
            return s;
        }
        ...
        function chooseOne(menuid) {
        	...
        	if(row){
        		var url = "<%=path%>/Examination/examinationBodyAdd.page?1=1&tempid="+tempid+"&crimid="+row.crimid+"&menuid="+menuid+"&flowdefid=${flowdefid}"+"&lastnodeid="+row.duty;
        	...
        	var rows = grid.getSelecteds();
           	if(rows.length > 0){
				   ...
				   if(ids.length == '0'){//所选中的数据都是审批中的
						   ...
						   var id = ids.join(',');
		                   var url = "<%=path%>/Examination/examinationBodyAdd.page?1=1&tempid="+tempid+"&id="+id+"&menuid="+menuid+"&flowdefid=${flowdefid}";
		                   self.location.href=url;
				   		}
				   }
           	}else {
