...
<body>
	...
	<div size="38px;" showCollapseButton="false">
		...
		<td style="width:100%;">
	    	...
	    	${topstr}
	    	<!--  
	    	<a class="mini-button" style="display:none;" id="11121" iconCls="icon-save" plain="true" onclick="savedata();">存盘</a>
	    	-->
    ...
    <script type="text/javascript">
      ...
      //保存或更新
        function savedata() {
        	...
        	if(operator == "new"){//新增
	            $.ajax({
	                url:"<%=path%>/toInform/saveToInform.json?1=1",
	                data: {content:content,tempid:tempid,operator:operator,noteinfo:noteinfo},
	                type: "post",
            ...
            if(operator == "edit"){//更新
	            $.ajax({
	                url: "<%=path%>/toInform/saveToInform.json?1=1",
	                data: {content:content,docid:docid,operator:operator,noteinfo:noteinfo},
	                type: "post",
	                success: function (text) {
	                	alert("操作成功!");
	                	close();
