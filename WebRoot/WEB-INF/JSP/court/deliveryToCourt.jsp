...
<body onload="init('${menuid}');">
    ...
    <div id="datagrid" class="mini-datagrid" style="width:100%;height:100%;" allowResize="false"  sizeList="[10,20,30,50,100]"
       url="<%=path%>/courtJoint/deliveryToCourtList.action?1=1&flowdefid=${flowdefid}" onrowdblclick="chooseOne" 
       idField="spid" multiSelect="true" pageSize="20" allowAlternating="true"  virtualScroll="true" onselectionchanged="selectionChanged">
        <div property="columns">
            ...
            <div field="XINGMING" width="30px;" headerAlign="center"  align="center" allowSort="false" >姓名</div>  
         	<div name="action" width="100px" headerAlign="center" align="center" renderer="uploadWrit" cellStyle="padding:0;">上传送达回证</div>
          </div>
    </div>
  </div>
</div> 
  ...
  <div style="height: 0px;">
	...
	<script type="text/javascript" src="<%=path%>/scripts/fileupload/ajaxfileupload.js"></script>
    ...
    <script type="text/javascript">
        ...
        function search() {
 	      ...
 	      var picibianhao = mini.get("picibianhao").getValue();
 	       ...
 	       grid.load({curyear : curyear, picibianhao:picibianhao});
       }
...
/*         function onKeyEnter(e) {
        ...
        function uploadWrit(e) {
      	  
            var grid = e.sender;
            var record = e.record;
            var uid = record._uid;
            var rowIndex = e.rowIndex;
            var s =  ' <form id="uploadform" action="" method="post" enctype="multipart/form-data"  >'
            	　　　+'<input type="file" id="file" name="file" class="mini-fileupload" />&nbsp&nbsp&nbsp&nbsp'
            	　　　+'<input  class="mini-button" type="button" value="上传送达回证" onclick="beforeSubmit()"></input></form>'
        ...
        function beforeSubmit(){
	            ...
	            var url = "<%=path%>/courtJoint/uploadDeliveryWrit.action?crimid="+crimid+"&batchid="+batichid;
        ...
        function createXml(){
  	      ...
  	      var crimids = [];
  			...
  			$.ajax({
   			    url: "<%=path%>/courtJoint/SendReceiptToCourttest.action?crimids="+crimids+"",
  		        data: {curyear:curyear,picibianhao:picibianhao},
  		        type: "POST",
  		        dataType:"json",
  		        async:false,
...
<%--         function uploadDeliveryWrit(uid){
