...
<html>
	<head>
		...
		<title>数据交换dataexchangemanage.jsp</title>
			...
			<table style="width:98%;">
				<tr>
					...
					<input id="ddcid" name="ddcid" class="mini-combobox" 
	  				  style="width:180px;" popupWidth="450"  textField="ddcname" valueField="ddcid"
					...
					&nbsp; &nbsp;
					<div id="isCallPrec" class="mini-checkbox" checked="false" readOnly="false" text="执行存储过程"></div>&nbsp; &nbsp;
					<select name="insertonly"  id="insertonly" style="width:90px;">
						<option value="0" selected="selected">更新优先</option>
						<option value="1">尝试插入</option>
					</select>
					&nbsp;&nbsp;&nbsp;
					<a class="mini-button" iconCls="icon-ok" onclick="addExchangeDataRequest()" plain="true">执行</a>
					
					&nbsp;&nbsp;&nbsp;
					<!-- <a class="mini-button" iconCls="icon-user" onclick="exchangeDataDept()" plain="true">自动交换本单位所有数据方案</a>
					 -->
					&nbsp;&nbsp;
					</td>
					<td align="right">
	...
	<div class="mini-toolbar  filter_div  hide"  style="padding:2px;border:1;">
	     ...
	     <div id="datagrid_condition" style="width:100%;height:100px;" class="mini-datagrid" 
	    	pageSize="2000"
	    	allowResize="false" 
	    	showPager="false" showLoading="true" allowRowSelect="true"
	    	showFilterRow="false" 
	    	>
	        <div property="columns">
	        		...
	        		<div field="relationtype" name="relationtype" renderer="onRelationtypeRenderer" width="40" headerAlign="center" align="left"  allowSort="true">
               		...
               		<div field="_sp2" name="_sp2" width="180" headerAlign="center" align="left"></div>
	        </div>
	    </div>
    </div>
	...
	<div class="mini-fit">
		<div align="left" style="padding-left: 100px;">
			...
			<hr width="300px" align="left"/>
			<div>或者点击: 自动交换本单位所有数据方案, 根据配置执行交换</div>
	...
	<script type="text/javascript">
		...
		function getAndCheckDdcid(showalert){
			var $ddcid = mini.get("ddcid");
	        // 变量会自动提前
			var ddcid = null;
		    if($ddcid){
	        	ddcid = $ddcid.getValue();
		    }
        	// 
        	if(!ddcid && showalert){
        		alert("请选择数据交换方案!");
		...
		function deleteRow(){
		    ...
		    if (row) {
		    	...
		    	if(del){
		        	datagrid_condition && datagrid_condition.removeRow(row);
		    	}
		    }
		};
		...
		// 添加导出请求
		function addExchangeDataRequest(){
			//
		    var ddcid = getAndCheckDdcid(1);
			//
			if(!ddcid){
			...
			//
			var condition = getConditionmessage();
			//var insertonly = "1";
			var insertonly = getInsertOnly("0");
			
			var isCallPrec = mini.get("isCallPrec").getChecked();
		    var data =  {
		    	ddcid: ddcid
		    	, condition : condition
		    	, insertonly : insertonly
		    	, queryuuid : queryuuid
		    	, isCallPrec : isCallPrec
		    };
			//
			var  url = "<%=path%>/dbms/ajax/calldataexchange.json";
		...
		//
		function exchangeDataDept(){
			//
			var manual = "1";
			var condition = "";
			var insertonly = "1";
			// 将 queryuuid 作为key,查询进度
			var queryuuid = "uuid"+Math.random();
			window.queryuuid = queryuuid;
			//
		    var data =  {
				manual : manual
