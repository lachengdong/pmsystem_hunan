...
<html>
				...
				<tr>
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="mini-button" iconCls="icon-reload"
							 onclick="refresh()" plain="true">手动刷新</a>
			            &nbsp;&nbsp;						
						<a class="mini-button" iconCls="icon-download"
							 onclick="batchUnzip()"  plain="true">批量解压</a>
			            &nbsp;&nbsp;
					</td>
				</tr>
			</table>
    ...
    <div class="mini-fit">
	    <div id="datagrid1" style="width:100%;height:100%;" class="mini-datagrid" 
	    	url="<%=path %>/dbms/getZipListInfoData.json" 
    ...
    <script type="text/javascript">
        ...
        function onActionRenderer(e) {
            var s = '<a class="" href="javascript:batchUnzip()" >解压</a>&nbsp;&nbsp;&nbsp;&nbsp;';
                s += '&nbsp;&nbsp;<a class="" href="javascript:deleteRow()">删除</a>'; 
            return s;
        };
        
        function refresh(){
        	var del = confirm("确实要刷新吗?");
	    	if(!del){
		    	return false;
	    	}
	    	var data = {};
	    	progressDesc();
	    	//grid.loading("正在刷新，请稍候...");
			var  url = "<%=path%>/dbms/refresh.json";
		...
		function batchUnzip(){
			...
			// 界面
			//grid.loading("正在解压文件，请稍候...");
			progressDesc();
			var  url = "<%=path%>/dbms/batchUnzip.json";
			var successCallback = function (message) {
	        	   var info = message["info"];
	               if(1 === message["status"]){
			    		// 判断是否成功
