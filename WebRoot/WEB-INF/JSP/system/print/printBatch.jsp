...
<body >
	<div  id="form1" class="mini-splitter"  vertical="true" style="width:100%;height:100%;border:0px;">
		<div size="20px;" showCollapseButton="false">
			<input class="mini-hidden" id="provincecode" name="provincecode" value="${provincecode}"/>
	        <div class="mini-toolbar" style="padding:2px;border-bottom:0;">
	                <a class="mini-button" id="dakai" iconCls="icon-lock" plain="true" onclick="dakai()">工具栏</a> 
	                                                       案件号：(
	              		  <select id="caseyear" name="caseyear">
						       	<option value='${caseyear - 1}'>${caseyear - 1}</option>
						       	<option selected='selected' value='${caseyear}'>${caseyear}</option>
						       	<option value='${caseyear + 1}'>${caseyear + 1}</option>
					      </select>                 
	                )
   ...
   function dayin(text){
       ...
       function daochu(text){//导出
        	...
        	//printscheme
        	var selectIndex = document.getElementById("printscheme").selectedIndex;//获得是第几个被选中了
			var selectText = document.getElementById("printscheme").options[selectIndex].text ;//获得被选
			...
			for(var j=0;j<objs.length;j++){ 
				...
				if(aipObj.IsOpened()){
					var filename = "(" + objs[j].niandu + ")" + shortname + casetypetext + "第" + objs[j].anhao + "号"+selectText;
