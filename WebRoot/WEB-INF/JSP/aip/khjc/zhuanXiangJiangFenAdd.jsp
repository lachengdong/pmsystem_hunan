...
<body onload="init('${menuid}');">
	...
	<div size="38px;" showCollapseButton="false">
		...
		<td style="width:100%;">
			...
			<a class="mini-button"  iconCls="icon-save" plain="true" style="display:none;" id="11220" onclick="savedata();">存盘</a>
	    	<a class="mini-button"  iconCls="icon-save" plain="true" style="display:none;" id="11221" onclick="savedata();">存盘</a>	
    ...
    <script type="text/javascript">
      ...
      //保存或更新
        function savedata(operateType,templetid,flowdefid,state) {
       		//表单所有节点及值
			...
			var docid = document.getElementById("docid").value;
       		var url="saveCriminalScoreInfo.action?1=1";
            $.ajax({
                url:encodeURI(encodeURI(url)),
                data: {content:content,operateType:operateType,docid:docid,templetid:templetid,flowdefid:flowdefid,state:state,noteinfo:noteinfo+""},
                type: "post",
        ...
        function saveMeetingdata(operateType,templetid) {
       		//表单所有节点及值
			var noteinfo = getNoteMap();
			//获取大字段信息
			var aipObj=document.getElementById("HWPostil1");
			var content = aipObj.GetCurrFileBase64();
			var docid = document.getElementById("docid").value;
       		var url="saveMeetingInfo.action?1=1";
            $.ajax({
                url:encodeURI(encodeURI(url)),
                data: {content:content,operateType:operateType,docid:docid,templetid:templetid,noteinfo:noteinfo+""},
                type: "post",
                success: function (text) {
                	alert("操作成功!");
                	onCancel();
