...
<body onload="init('${menuid}');">
    ...
    <script type="text/javascript">
       mini.parse();
        ...
        //保存、提交
        function doSubmitFlow(menuid,operateType,flowdefid,tempid){  
        	var crimid = mini.get("crimid").getValue();
     		var aipObj = document.getElementById("HWPostil1");
			var points = aipObj.GetValueEx("points",2,"",0,"").replace(/[ ]/g,"");
			var pljkfnames = aipObj.GetValueEx("pljkfnames",2,"",0,"").replace(/[ ]/g,"");
			if(!points) {
				 alert("奖扣分值不能为空！");
				 return ;
			}else if(!pljkfnames && !crimid) {
				 alert("罪犯姓名不能为空！");
				 return ;
			}else{
				var flowdraftid = '';  
				...
				returnLockUserMain(flowdraftid,menuid,operateType,flowdefid,tempid,0);
			}
        }
        function saveNext(){
       		back();
