...
<body  onload="myload()">     
			...
			<div size="70%" showCollapseButton="true"> 
    			<form id="form1" method="post">
    				<div class="mini-toolbar" style="border-bottom:0;padding:1px;height:30px;" >
			           ...
			           <a class="mini-button" onclick="onOk1" plain="true" style="width:80px">采纳意见</a>      
			           <a class="mini-button" onclick="onCancel"  plain="true" style="width:60px;">取消</a>
    				</div>
    				
    				<div style="padding-left:11px;padding-bottom:6px;padding-right:11px;float:left;">
			            <table style="table-layout:fixed;">
							...
							<tr>
					        	<td colspan="5" rowspan="3">    
					            	<input id="reason" name="reason" class="mini-textarea" allowInput="false" emptyText="" style="width:100%; height: 70px" />
					            	<input id="xgreason" name="xgreason" class="mini-textarea" emptyText="请填写修改意见的原因" style="width:100%; height: 70px" />
					            </td>
			            	</tr>
			            </table>
			        </div>
	   		...
	   		<div showCollapseButton="true">
	             <fieldset style="width:95%;height:89%;">
			        <legend>系统审查:</legend>
			        <div  style="padding:5px;" >
			         	<table >
		                    <tr>
		                        <td colspan="5" rowspan="5">    
		                            <span id="scyj" style="padding-left:10px;"></span>
		                        </td>
		                    </tr> 
                		</table> 
			        </div>
    ...
    <script type="text/javascript">
        ...
        var enableEditNode = ['gdjysuggest1','gdjysuggest3','gdjysuggest5'];//控制理由框是否可编辑的意见节点
        var isReasonEdit = arrContains(enableEditNode, strname);
        if(isReasonEdit){
        	mini.get("reason").setAllowInput(true);
        }
       ...
       function myload(){
		    ...
		    jxbasis = jxbasis + showDocInfo();
			document.getElementById('scyj').innerText=jxbasis;
	      } 
	       ...
	       function MtoN(range){
	         var result="";
	       	 if(range&&(range!=0)&&range!=null){
	       	 	if(range>=12){
	       	 	    result+= mycars[parseInt(range/12)]+"年"; 
	       	 		if((range%12)!=0){
	       	 			result+=mycars[range%12]+"个月 ";
	       	 		}
	       	 	}else{
	       	 		result+=mycars[range]+"个月";
	       	 	}
	       	 }else{
	       	 	result+="一年";
        ...
        function onOk1() {
            ...
            s10.setValue("");
            s11.setValue("");
			var result = setReason();//根据所选数据生成减刑意见
			var reason = mini.get("reason").value;
            ...
            row[14] = reason;
            row[15]= result;
            if(s1.getValue()!=1)row[16]=result_date;
            window.returnValue = row;
            CloseWindow("cancel");
        }
        function onOk2() {
        	var result = setSuggest();//根据所选数据生成减刑意见
        	var xgreason = mini.get("xgreason").getValue();
        	
            if(s15.getValue()==0&&s17.getValue().trim()==''){
            	alert("必须填写否决意见！");
            	return;
            }
            if(s15.getValue()==0){
      ...
      function result_NYR(N,Y,R){
        ...
        function setReason(){
            ...
            var s12text = s12.getValue();				
      		if(s1value==1){
      			tempids = replaceAllStr(tempids,'(减刑假释)','假释');
      	        //tempids = tempids.replace('(减刑假释)','假释');
      	    }else{
      	    	tempids = replaceAllStr(tempids,'(减刑假释)','减刑');
      	        //tempids = tempids.replace('(减刑假释)','减刑');
      	    }
			var result = setSuggest();
			//alert(result);
			tempids = tempids.replace("@", result).replace("GK054", s8.text);
			if(s11text== '1'){
				resultinfo = "" ;
				resultinfo = "符合从宽"+s7text+",";
				}
			if(s12text == '1'){
				resultinfo = "" ;
				resultinfo = "符合从严"+s9text+",";				
				} 
			if(s11text== '1' && s12text == '1'){
				resultinfo = "符合从严:"+s9text+","+"符合从宽:"+s7text+",";	
				}
			tempids = tempids.replace("@", result).replace("GK054", s8.text).replace("CY001", "").replace("CK001", resultinfo);
	            		...
	            		if(flag){//不是有期的
		            		...
		            		if(parseInt(s2value) < 26){//有期
	            				...
	            				//返回总结果 
			            			...
			            			result_date = result_NYR(resultN,resultY,resultD);
			            			//alert(result_date+"zongyu daowule ");
			            			if(flag_N&&resultN>0){
			            				resultN = mycars[resultN];
			            				result +=resultN+"年";
			            			}
