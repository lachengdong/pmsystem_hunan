...
<body>
		<div id="tabs" class="mini-tabs" name="aaa" activeIndex="0"   style="width:100%;height:100%;" bodyStyle="padding:0;border:0;">
			...
			<!-- 12498 减刑假释建议书专用ID ，减刑假释建议书查询方案必须绑定此资源ｉｄ -->
			<div  id="tab3"  name="3"  title="建议书" url="toSuggestionDocPage.action?1=1&crimid=${crimid}&flowdraftid=${flowdraftid}&flowid=${flowid}&flowdefid=${flowdefid}&menuid=${menuid}&doctype=jailcommute&resid=12498" refreshOnClick="true" visible="true">
			</div>
			<!--
			...
			-->
			<div  id="tab4"  name="4"   title="基本信息" url="basicInfo/basicInformation.page?1=1&crimid=${crimid}&closetype=${closetype}&fathermenuid=${fathermenuid}"  refreshOnClick="true"  visible="true">
			</div>
			<c:if test="${danganziliao eq 1}">
                 <div  id="tab5"  name="5"  title="档案资料" url="toArchivalDataDocumentPage.page?1=1&crimid=${crimid}&flowdraftid=${flowdraftid}&flowid=${flowid}&menuid=${menuid}" refreshOnClick="true"  visible="true"></div>
            </c:if>
            <!--
			...
			<div  id="tab4"  name="5"   title="刑期变动" url="toSentenceChangeListPage.page?1=1&crimid=${crimid}&toolbar=0"  refreshOnClick="true"  visible="true">
			</div>
			<c:if test="${provinceid eq 4300}">
			<div  id="tab5"  name="5"   title="台账" url="<%=path %>/detainRegister.action?1=1&menuid=18212&crimid=${crimid}&toolbar=0"  refreshOnClick="true"  visible="true">
			</div>
			</c:if>
...
<!--			</div>-->
<!--			<div  id="tab8"  name="8"  title="呈批表" url="toChengPiReportPage.page?1=1&crimid=${crimid}&flowdraftid=${flowdraftid}&flowid=${flowid}" refreshOnClick="true"  visible="true">-->
<!--			</div>-->
