...
<head>
	...
	<body onload="init('${menuid}');">
		<div class="mini-toolbar" style="padding: 2px; border-bottom: 0px;">
			...
			<input name="tempid" id="tempid" class="mini-hidden" value="SZ_JXJS_JWZXGZS" />
			<table>
				<tr>
					<td style="width: 100%;">
						 <a class="mini-button" style="display:none;" id="11842" iconCls="icon-add" plain="true" onclick="add('11842')">新增</a>
						 <a class="mini-button" iconCls="icon-download" plain="true" style="display:none;" id="12867" onclick="saveNext()">下一个</a>
		...
		<div id="datagrid" allowMoveColumn="false" onrowdblclick="rowdblclickfunction" style="width: 100%; height: 100%;" class="mini-datagrid" sizeList="[10,20,50,100]" pageSize="20" 
			allowAlternating="true" showLoading="true" allowResize="false" url="getprisonExecuteDecisionManage.action?1=1&tempid=SZ_JXJS_JWZXGZS&crimid=${crimid}" multiSelect="true">
	...
	<script type="text/javascript">
	    ...
	    function add(id){
	    	...
	    	mini.open({
                url: "addprisonExecuteDecisionManage.action?1=1&tempid="+tempid+"&crimid="+crimid+"&menuid="+id,
                showMaxButton: true,
             	allowResize: false, 
                title: "暂予监外执行管理事项告知书", width: 900, height: 500,
                onload: function () {
		...
		function chakan(menuid) {
	    	...
	    	mini.open({
                url: "addprisonExecuteDecisionManage.action?1=1&docid="+row.docid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
                title: "暂予监外执行管理事项告知书", width: 900, height: 500,
                onload: function () {
        ...
        function shanchu(){
            ...
            if (row) {
                if (confirm("确定删除选中记录？")) {
                    $.ajax({
                        url: "deleteprisonExecuteDecisionManage.action?1=1&docid="+row.docid,
                        type: "post",
         ...
         function onCancel(){  
	        url = "prisonExecuteDecisionManageChoose.action?1=1&menuid=10291";
	        self.location.href=url;          
     	 }
    </script>
	</body>
