...
<body onload="init('${menuid}');">
     <div class="mini-toolbar"  style="padding:2px;border-bottom: 0px;">
        ...
        <input id="name" name="name" class="mini-hidden" value="${name}"/>
          <table >
             <tr>
              <td style="width:100%;">
              	<a class="mini-button" style="display:none;" id="12447" iconCls="icon-add" plain="true" onclick="add('12447')">新增</a>
              	<!-- <a class="mini-button" style="display:none;" id="12381" iconCls="icon-remove" plain="true" onclick="shanchu()">批量删除</a> -->
              	 <a class="mini-button" style="display:none;" id="13728" iconCls="" plain="true" class="mini-hidden" onclick="chooseNext();">下一个</a> 
    ...
    <div class="mini-fit">
	    ...
	    allowResize="false" url="<%=path%>/shSolicitOpinions/getSolicitOpinionsList.json?1=1&tempid=${tempid}&crimid=${crimid}"  
    ...
    <script type="text/javascript"> 
     ...
     function add(menuid){
        ...
        var tempid=mini.get("tempid").getValue();
  		mini.open({
  	        url: "shSolicitOpinions/solicitOpinionsAdd.page?1=1&tempid="+tempid+"&crimid="+crimid+"&menuid="+menuid,
  	        showMaxButton: true,
  	        allowResize: false,
  	        title: "保外就医征求意见书", width: 900, height: 500,
  	        onload: function () {
   ...
   //删除
     function shanchu(){
         ...
         if (rows.length > 0) {
             if (confirm("确定删除选中记录？")) {
                 ...
                 var id = ids.join(',');
                 $.ajax({
                     url: "<%=path%>/shSolicitOpinions/deleteSolicitOpinions.action?1=1&docid="+id,
                     type: "post",
 	...
 	function xiugai(menuid) {
 		...
 		var crimid = mini.get("crimid").getValue();
     	mini.open({
             url: "shSolicitOpinions/editSolicitOpinions.page?1=1&docid="+row.docid+"&menuid="+menuid,
             showMaxButton: true,
          	 allowResize: false, 
             title: "保外就医征求意见书", width: 900, height: 500,
             onload: function () {
 	...
 	function chakan(menuid) {
 	 ...
 	 //关闭返回到罪犯处理页面
    function close(){
        url = "/pmsys/shSolicitOpinions/solicitOpinionsChoose.page?1=1";
        self.location.href=url;
    }
    function chooseNext(){
		var url="<%=path%>/shSolicitOpinions/solicitOpinions.page?1=1";
		var tempid=mini.get("tempid").getValue();
