...
<head>
    <title>原厅局级以上职务犯罪罪犯减刑、假释 暂予监外执行情况登记表</title>
...
<body onload="init('${menuid }')">
	...
	<div size="38px;" showCollapseButton="false">
		...
		<td style="width:100%;">
	    	...
	    	<a class="mini-button" id="14069"  style="display:none;" iconCls="icon-save" plain="true" onclick="savedata();">存盘</a>
	    	<a class="mini-button" id="14070"   style="display:none;" iconCls="icon-download"   plain="true" onclick="saveNext();">下一个</a>
    ...
    <script type="text/javascript">
        ...
        function savedata() {
       		...
       		var url="saveCriminalFileTransfer.action?1=1";
        	if(operator == "new"){//新增
	            $.ajax({
            ...
            if(operator == "edit"){//更新
	            $.ajax({
	                url: "saveCriminalFileTransfer.action?1=1",
	                data: {content:content,docid:docid,operator:operator},
	                type: "post",
	                success: function (text) {
	                	alert("操作成功!");
	                  	back();
         ...
         function saveNext(){
            ...
            var url = "DutyCriminalRegistrationFormList.action?1=1&id="+id+"&menuid="+menuid+"&tempid=SH_ZYJWZXQKDJB";
            if(index>0){
         ...
         //跳转到选择罪犯页面
 	   function onCancel(){
	    	var url = "TheDutyCriminalRegistrationForm.action?1=1&id="+id+"&menuid=14066";
			self.location.href=url;
       }
		//标准方法接口定义
