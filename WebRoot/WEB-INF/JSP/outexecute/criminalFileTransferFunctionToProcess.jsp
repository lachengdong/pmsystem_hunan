...
<body onload="init('${menuid }')">
      <div class="mini-toolbar"  style="padding:2px;border-bottom: 0px;">
	  	   <input name="tempid" id="tempid" class="mini-hidden" value="JWZX_ZFDAZDH"/>
	  	   <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
           <table >
              <tr>
               	 <td style="width:100%;">
               	 	 <a class="mini-button" style="display:none;" id="12716" iconCls="icon-add" plain="true" onclick="add('12716')">新增</a>
              	     <a class="mini-button" style="display:none;" id="12886" iconCls="icon-download" plain="true" onclick="saveNext();">下一个</a>
     ...
     <div class="mini-fit">
	    <div id="datagrid" allowMoveColumn="false" onrowdblclick="rowdblclickfunction" style="width:100%;height:100%;" class="mini-datagrid" sizeList="[10,20,50,100]" pageSize="20"
	     	 allowAlternating="true" showLoading="true" allowResize="false" url="getCriminalFileTransferFunctionTableList.action?1=1&tempid=JWZX_ZFDAZDH&crimid=${crimid }" multiSelect="true" >
	...
	<script type="text/javascript">
	    ...
	    function add(menuid){
	    	...
	    	mini.open({
                url: "addCriminalFileTransferFunctionForm.action?1=1&tempid="+tempid+"&crimid="+crimid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
                title: "罪犯档案转递函", width: 900, height: 500,
                onload: function () {
		...
		function chakan() {
	    	...
	    	mini.open({
                url: "editorlookCriminalFileTransferFunction.action?1=1&docid="+row.docid,
                showMaxButton: true,
             	allowResize: false, 
                title: "罪犯档案转递函", width: 900, height: 500,
                onload: function () {
		...
		function xiugai(menuid) {
			var row = grid.getSelected();//获取选中记录
	    	mini.open({
                url: "editorlookCriminalFileTransferFunction.action?1=1&docid="+row.docid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
                title: "罪犯档案转递函", width: 900, height: 500,
                onload: function () {
        ...
        function shanchu(){
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        url: "deleteCriminalFileTransferFunction.action?1=1&docid="+id,
                        type: "post",
		...
		function saveNext (){
	        ...
	        var url= "criminalFileTransferFunctionToProcess.action?1=1&id="+id+"&menuid=${menuid}&idname="+idname;
	        if(index>0){
	 	 ...
	 	 //关闭返回到罪犯处理页面
	    function close(){
	        url = "criminalFileTransferFunction.action?1=1";
	        self.location.href=url;
	    }
    </script>
</body>
