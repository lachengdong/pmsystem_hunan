...
<body onload="init('${menuid}');">
       ...
       <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
            <table >
               <tr>
                <td style="width:100%;">
            	 	<a class="mini-button" iconCls="icon-add" plain="true" style="display:none;" id="1000144" onclick="add('1000144');">新增</a>
           			<a class="mini-button" iconCls="icon-download" plain="true" style="display:none;" id="13873" onclick="saveNext()">下一个</a>
    ...
    <div class="mini-fit">
	    ...
	    url="getZFBCJDBListByCrimid.action?1=1&crimid=${crimid}&tempid=ZFBCJDB"  idField="" multiSelect="true" allowAlternating="true" showLoading="true">
...
<!--	       		<div type="checkcolumn" width="15px"></div>-->
    ...
    <script type="text/javascript"> 
        ...
        function add(menuid){
	 		...
	 		mini.open({
	 	        url: "toZFBCJDBAdd.action?1=1&tempid=ZFBCJDB&crimid="+crimid+"&menuid="+menuid+"&name="+name,
	 	        showMaxButton: true,
	 	        allowResize: false,
		...
		function chakan(menuid) {
			...
			var crimid = mini.get("crimid").getValue();
	    	mini.open({
                url: "ZFBCJDBLook.action?1=1&docid="+row.docid+"&crimid="+crimid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
        ...
        function shanchu(){
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        url: "ZFBCJDBDelete.action?1=1&docid="+id,
                        type: "post",
		 ...
		 function saveNext (){
	        ...
	        var url= "toZFBCJDBDocList.action?1=1&id="+id+"&menuid=${menuid}&idname="+idname;
	        if(index>0){
		...
		//返回到罪犯处理页面
     function onCancel(){  
        url = "toZFBCJDBChoose.action?1=1";
        self.location.href=url;          
     }
</script>
</body>
