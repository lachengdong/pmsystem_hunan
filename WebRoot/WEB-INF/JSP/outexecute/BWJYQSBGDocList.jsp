...
<body onload="init('${menuid}');">
       ...
       <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
            <table >
               <tr>
                <td style="width:100%;">
            	 	<a class="mini-button" iconCls="icon-add" plain="true" style="display:none;" id="1000135" onclick="add('1000135');">新增</a>
<!--           			<a class="mini-button" iconCls="icon-download" plain="true" style="display:none;" id="1000136" onclick="saveNext()">下一个</a>-->
<!--                	<a class="mini-button" iconCls="icon-close"  plain="true" onclick="onCancel();" >关闭</a>-->
    ...
    <div class="mini-fit">
	    ...
	    url="getBWJYQSBGListByCrimid.action?1=1&crimid=${crimid}&tempid=BWJYQSBG"  idField="" multiSelect="true" allowAlternating="true" showLoading="true">
...
<!--	       		<div type="checkcolumn" width="15px"></div>-->
    ...
    <script type="text/javascript"> 
        ...
        function add(menuid){
	 		...
	 		mini.open({
	 	        url: "toBWJYQSBGAdd.action?1=1&tempid=BWJYQSBG&crimid="+crimid+"&menuid="+menuid+"&name="+name,
	 	        showMaxButton: true,
	 	        allowResize: false,
		...
		function chakan(menuid) {
			...
			var crimid = mini.get("crimid").getValue();
	    	mini.open({
                url: "BWJYQSBGLook.action?1=1&docid="+row.docid+"&crimid="+crimid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
                title: "罪犯暂予监外执行取保书", width: 900, height: 500,
                onload: function () {
                    var iframe = this.getIFrameEl();
		...
		function xiugai(menuid) {
	    	...
	    	mini.open({
                ...
                showMaxButton: true,
             	allowResize: false, 
                title: "罪犯暂予监外执行鉴定考察表", width: 900, height: 500,
                onload: function () {
                    var iframe = this.getIFrameEl();
        ...
        function shanchu(){
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        url: "BWJYQSBGDelete.action?1=1&docid="+id,
                        type: "post",
		 ...
		 function saveNext (){
	        ...
	        var url= "toBWJYQSBGDocList.action?1=1&id="+id+"&menuid=${menuid}&idname="+idname;
	        if(index>0){
		...
		//返回到罪犯处理页面
     function onCancel(){  
        url = "toBWJYQSBGChoose.action?1=1";
        self.location.href=url;          
     }
</script>
</body>
