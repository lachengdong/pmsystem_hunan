...
<html>
	<head>
		...
		<title>罪犯暂予监外执行社区矫正意见书文书列表</title>
		<style type="text/css">
    body{
...
<body onload="init('${menuid}');">
       ...
       <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
            <table >
               <tr>
                <td style="width:100%;">
            	 	<a class="mini-button" iconCls="icon-add" plain="true" style="display:none;" id="1000098" onclick="add('1000098');">新增</a>
           			<a class="mini-button" iconCls="icon-download" plain="true" style="display:none;" id="1000099" onclick="saveNext()">下一个</a>
    ...
    <div class="mini-fit">
	    ...
	    url="getJZYJSListByCrimid.action?1=1&crimid=${crimid}&tempid=JZYJS"  idField="" multiSelect="true" allowAlternating="true" showLoading="true">
...
<!--	       		<div type="checkcolumn" width="15px"></div>-->
    ...
    <script type="text/javascript"> 
        ...
        function add(menuid){
	 		...
	 		mini.open({
	 	        url: "toJZYJSAdd.action?1=1&tempid=JZYJS&crimid="+crimid+"&menuid="+menuid+"&name="+name,
	 	        showMaxButton: true,
	 	        allowResize: false,
		...
		function chakan(menuid) {
			...
			var crimid = mini.get("crimid").getValue();
	    	mini.open({
                url: "JZYJSLook.action?1=1&docid="+row.docid+"&crimid="+crimid+"&menuid="+menuid,
                showMaxButton: true,
             	allowResize: false, 
        ...
        function shanchu(){
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        url: "JZYJSDelete.action?1=1&docid="+id,
                        type: "post",
		 ...
		 function saveNext (){
	        ...
	        var url= "toJZYJSDocList.action?1=1&id="+id+"&menuid=${menuid}&idname="+idname;
	        if(index>0){
