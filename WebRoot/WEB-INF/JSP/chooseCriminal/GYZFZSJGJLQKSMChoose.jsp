...
<body onload="init('12426')">
    <div class="mini-toolbar"  style="padding:0px;border-bottom: 0px;">
    	<jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
        <table >
             <tr>
                <td style="width:100%;">
                <a class="mini-button" style="display:none;" id="12434" plain="true" onclick="chooseAll(12427);">批量处理</a>
                </td>
    ...
    <div class="mini-fit" id="div_two">
        <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" allowResize="false" sizeList="[10,20,50,100]" pageSize="20"
        	 url="getGYZFZSJGJLQKSMChooseList.action?1=1" idField="spid" multiSelect="true" allowAlternating="true" virtualScroll="false" 
    ...
    <script type="text/javascript">
         ...
         function onActionRenderer(e) {
        	...
        	if(record.idnumber && record.duty != '-1' ){
        		if(record.duty=='0' || record.accent=='1'){
        			s += ' <a class="Edit_Button"  href="javascript:chooseOne(12427);" >修改</a>';
        		}else {
        			s += ' <a class="Edit_Button"  href="javascript:chooseOne(12428);" >查看</a>';
        		}
        	}else{
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(12427);" >处理</a>';
        	} 
            return s;
        }
       //根据罪犯编号跳转到表单页面，如果寻找罪犯未处理或者审批未通过,没有流程idnumber,生成一条新数据。
        function chooseOne(menuid) {
        	...
        	if(row){
        		var url = "showGYZFZSJGJLQKSM.action?1=1&crimid="+row.crimid+"&menuid="+menuid;
        	...
        	var rows = grid.getSelecteds();
           	if(rows.length > 0){
				   ...
				   if(ids.length == '0'){//所选中的数据都是审批中的
						   ...
						   var id = ids.join(',');
		                   var url = "showGYZSJGJLQKSM.action?1=1&id="+id+"&menuid="+menuid;
		                   self.location.href=url;
				   		}
				   }
           	}else {
