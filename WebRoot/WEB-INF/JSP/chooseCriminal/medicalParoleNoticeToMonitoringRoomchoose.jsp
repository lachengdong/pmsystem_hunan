...
<body onload="init('10285')">
	
    <div class="mini-toolbar"  style="padding:0px;border-bottom: 0px;">
    <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
    <!-- 隐藏字段，判断选择罪犯后跳转到哪个主页面 -->     
    <input id="menuid" name="menuid" type="hidden" value="${menuid}"/>
    <input id="tempid" name="tempid" class="mini-hidden" value="SZ_BWJY_TZSBJCS"/>
        <table >
             <tr>
                <td style="width:100%;">
                <a class="mini-button" style="display:none;" id="12792" plain="true" onclick="chooseAll('10285_01');">批量处理</a>	
                </td>
    ...
    <script type="text/javascript">
        ...
        grid.setUrl("getMedicalParoleNoticeToMonitoringRoomBasicInfoList.action?1=1");
        ...
        function onActionRenderer(e) {
	        ...
	        var s = '';
	        if(record.duty=='0'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'10285_01\');" >处理</a>';
        	}else if(record.duty=='1'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'10285_01\');" >修改</a>';
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'12795\');" >查看</a>';
        	}
            return s;
        }
        //新增或修改
        function chooseOne(menuid) {
        	...
        	var url = "medicalParoleNoticeToMonitoringRoomAdd.action?1=1&tempid="+tempid+"&crimid="+row.crimid+"&menuid="+menuid+"&systempid=${systempid}";
         ...
         function chooseAll(menuid){
            ...
            if (rows.length > 0) {
                if (confirm(allProcessing)) {
                    ...
                    var id = ids.join(',');
                    var url = "medicalParoleNoticeToMonitoringRoomAdd.action?1=1&id="+id+"&menuid="+menuid+"&systempid=${systempid}";
                    self.location.href=url;
                }
            } else {
