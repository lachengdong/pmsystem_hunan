...
<body onload="init('12308')">
    <div class="mini-toolbar"  style="padding:0px;border-bottom: 0px;">
    <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
    <input id="menuid" name="menuid" type="hidden" value="12187"/>
        <table >
             <tr>
                <td style="width:100%;">
                <a class="mini-button"   plain="true" onclick="chooseAll();">批量处理</a>
                </td>
    ...
    <div class="mini-fit" id="div_two">
        <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" allowResize="false" sizeList="[10,20,50,100]" pageSize="20"
        	 url="getZFBWJYSHWYHJLChooseList.action?1=1" idField="spid" multiSelect="true" allowAlternating="true" virtualScroll="false" 
    ...
    <script type="text/javascript">
       ...
       //根据审批状态显示按钮
	        ...
	        var s = '';
        	if(record.accent=='0'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'12309\');" >处理</a>';
        	}else if(record.accent=='1'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'12309\');" >修改</a>';
        	}else if(record.accent=='2'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(\'12315\');" >查看</a>';
        	}
            return s;
        }
        //根据罪犯编号跳转到保外就医（收监执行）表单页面
		        function chooseOne(menuid) {
		            var menuid="12308";
		        	var row = grid.getSelected();
		        	var url = "showZFBWJYSHWYHJL.action?1=1&crimid="+row.crimid+"&menuid="+menuid+"&name="+row.name+"&orgid="+row.orgid1;
        ...
        function chooseAll(menuid){
            ...
            if (rows.length > 0) {
                if (confirm("确定操作 选中记录？")) {
                    ...
                    var id = ids.join(',');
                    var url = "showZFBWJYSHWYHJL.action?1=1&id="+id+"&menuid="+menuid;
                    self.location.href=url;
                }
            } else {
                alert( "请至少选中一条记录！");
