...
<head>
	<title>罪犯保外就医收监执行决定书(天津省局)</title>
...
<body onload="init('13862');">
    <div class="mini-toolbar"  style="padding:0px;border-bottom: 0px;">
    <jsp:include page="/WEB-INF/JSP/form/buttoncontrol.jsp"></jsp:include>
    <input id="tempid" name="tempid" class="mini-hidden" value="BWJYSJZXJDS"/>   
        <table >
             <tr>
                <td style="width:100%;">
				   <a class="mini-button" style="display:none;" id="13864" iconCls="" plain="true" onclick="chooseAll(13863)">批量处理</a>
                </td>
    ...
    <div class="mini-fit" id="div_two">
        <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" allowResize="false" sizeList="[10,20,50,100]" pageSize="20"
        	 url="getPrisonExecutionDecisionList.action?1=1&tempid=BWJYSJZXJDS" idField="spid" multiSelect="true" allowAlternating="true" virtualScroll="false" 
    ...
    <script type="text/javascript">
        mini.parse();
        var tempid = mini.get("tempid").getValue();//获取模版编号
        ...
        function onActionRenderer(e) {
	        ...
	        var s = '';
	        if(record.duty=='0'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(13863);" >处理</a>';
        	}else if(record.duty=='1'){
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(13863);" >修改</a>';
        		s += ' <a class="Edit_Button"  href="javascript:chooseOne(13867);" >查看</a>';
        	}
            return s;
        }
        function chooseOne(menuid) {
        	...
        	if(row){
        		var url = "PrisonExecutionDecision.action?1=1&crimid="+row.crimid+"&menuid="+menuid+"&tempid="+tempid;
         ...
         function chooseAll(menuid){
            ...
            if (rows.length > 0) {
                if (confirm("确定操作 选中记录？")) {
                    ...
                    var id = ids.join(',');
                    var url = "<%=path%>/PrisonExecutionDecision.action?1=1&id="+id+"&menuid="+menuid+"&tempid="+tempid;
                    self.location.href=url;
                }
            } else {
