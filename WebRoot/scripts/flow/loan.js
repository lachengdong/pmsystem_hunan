...
function AddCustAction(own, aipObj, baseUrl) {
	...
	// 页面数据初始化
	if (action == 'new') {
		myaipObj.SetValue("borrow_person", myown.$userName);
...
function setBillNo() {
	$.ajax({
				url : mybaseUrl + "/loan/ajax/getBillNo.json",
				type : "POST",
				dataType : "json",
				async : false,
				success : function(data) {
					myaipObj.SetValue("bill_no", '单据号：' + data.info);
				},
...
function doSetProject() {
	var url = mybaseUrl + "/loan/toLoanProject.page?1=1";
...
function doAddLoanInfo() {
	...
	parameter.billid=parameter.billid.replace("单据号：","");	
...
//start add by blue 2015-08-15
function beforeSaveAction(obj) {
	var project = obj.GetValueEx("project", 2, "", 0, "");

	if (project == "") {
		alert("请输入项目（如果‘项目’列表中没有数据，可以在预算管理中设置本部门的预算项目数据）！");
		return 0;
	}

	return 1;
}
//end add by blue 2015-08-15
