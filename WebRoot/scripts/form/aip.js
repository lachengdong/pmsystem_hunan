 ...
 * 
 * @param {string}
 *            container 加载AIP的容器id
 * @param {json}
 *            option 可选的参数，参考def的定义
 * @returns {AIP}
 */
function AIP(container, option) {
	...
	var cfg = {
		id : "0",
		prefix : "AIP_",
		codebase : "./download/RMViewer.cab",
		width : "100%",
		height : "100%",
...
AIP.prototype.Alert = function(msg) {
	if (typeof window._originalAlert === "function"
			|| typeof window._originalAlert === "object") {
...
AIP.prototype.Init = function() {
	var s = "";
	s += '<object id="' + this.cfg.prefix + this.cfg.id + '" style="width:'
			+ this.cfg.width + ';height:' + this.cfg.height + ';" codebase="'
			+ this.cfg.codebase + '#' + this.cfg.version
	...
	s += '</object>';
	s += '<input type="hidden" name="' + this.cfg.prefix + 'DATA_'
			+ this.cfg.id + '">';
...
AIP.prototype.OnCtrlReady = function() {
	...
	} else if (this.cfg.loadorigFileUrl != "") {
		this.obj.LoadOriginalFile(this.cfg.loadorigFileUrl, "doc");
		this.obj.ShowDefMenu = this.cfg.showMenu;
		this.obj.ShowScrollBarButton = this.cfg.showScrollBar;
		this.obj.ShowToolBar = this.cfg.showToolBar;	
		this.Login(this.cfg.loginUser, 1, 32767, "", "");
		this.obj.TrackRevisions = 1;
		this.obj.ShowRevisions = 0;
	}
		...
		if (value) {
			...
			if (typeof value == 'object') {
				...
				$.each(value, function(k, sign) {
					var signNote = tObj.InsertNote('sign_' + k + i, page, 3,
							StartX, StartY, xWidth, yHeight);
					tObj.SetValue(signNote, sign['content']);
					tObj.SetValue(signNote, ":PROP::LABEL:2");
					tObj.SetValue(signNote, ":PROP:HALIGN:"
									+ signContentPosition);
					...
					var userNote = tObj.InsertNote('user_' + k + i, page, 3,
							StartX - 5300, StartY + row * 720, xWidth, yHeight);
					...
					var timeNote = tObj.InsertNote('time_' + k + i, page, 3,
							StartX, StartY + row * 720, xWidth, yHeight);
 ...
 * @returns {undefined}
 */
AIP.prototype.Save = function(data, callback, mode) {
	var bRet = false;
	var msg = '';
	if (!(this.obj && this.obj.IsOpened && this.obj.IsOpened()))// 文档是否打开
	{
		...
		try {
			...
			if (data) {
				this.obj.HttpAddPostString("data", data); // 设置上传变量
			}
			...
			if (ret.code !== this.cfg.uploadReturnCode) {
				...
				if (this.cfg.pdfUploadUrl) {// 不再保存pdf，移动端直接打开版式文件
					...
					var tmpFileName = this.obj.GetTempFileName('pdf');
					if (tmpFileName) {
						this.obj.saveTo(tmpFileName, 'pdf', 0);
						this.obj.HttpAddPostFile('Attachment[pdf][]',
								tmpFileName);
						this.obj.HttpPost(this.cfg.pdfUploadUrl);
						this.obj.DeleteLocalFile(tmpFileName);
					}
				}
				// 生成缩略图
				if (this.cfg.thumbnailUploadUrl) {
					var tmpFileName = this.obj.GetTempFileName('jpg');
					if (tmpFileName) {
						this.obj.saveTo(tmpFileName, 'jpg', 0);
						this.obj.HttpAddPostFile('Attachment[thumbnail][]',
								tmpFileName);
						this.obj.HttpPost(this.cfg.thumbnailUploadUrl);
						this.obj.DeleteLocalFile(tmpFileName);
					}
				}
				// 保存页数
				if (this.cfg.pageUploadUrl) {
					$.post(this.cfg.pageUploadUrl, {
								'page' : this.obj.PageCount
							});
				}
				// 保存二维码
...
AIP.prototype.GetNotesData = function() {
	...
	var noteValue;
	while (noteName = this.obj.GetNextNote("", 0, noteName)) {
		noteType = this.GetNoteType(noteName);
		switch (noteType) {
			case this.noteType.textarea.value :
		...
		noteValue = this.GetNoteValue(noteName);
		data += '{name:"' + noteName + '", type:"' + noteTypeName
				+ '", value:"' + noteValue + '"},';
	}
	data += '}';
	return data;
...
AIP.prototype.Login = function(user, type, access, pass) {
	...
	if (ret !== 0) {
		if (type === 1) {
			this.obj.Login("HWSEALDEMO"+user, 4, 65535, "DEMO", "");			
		} else {
			this.Alert(this.GetErrDesc(ret));
		}
	}
...
AIP.prototype.ActHandWrite = function() {
	if (this.obj.IsLogin() === 0) {
		this.Login(this.cfg.loginUser, 1, 32767, "", "");
	}
	// this.obj.InsertNote("aa",0,2,10000,30000,30000,30000)) 添加手写域
	this.obj.CurrAction = 264;
};

AIP.prototype.ActAddSeal = function() {
	if (this.obj.IsLogin() === 0) {
		this.Login(this.cfg.loginUser, 1, 32767, "", "");
	}
...
AIP.prototype.ActPdf417code = function() {
	...
	$.ajax({
				...
				success : function(data) {
					if (data) {
						obj.InsertPicture(imgName, data.basedata, obj.CurrPage,
								34000, 46000, 458852);
					}
				}
			});
 ...
 * 打开Word文档
 */
AIP.prototype.Openfile = function() {
	var aipObj = this.obj;
	var s = aipObj.ShowDialog(0, "", "", "*.doc||*.docx");
	if (s != "")
		aipObj.LoadOriginalFile(s, "doc");
}

AIP.prototype.FullScreen = function() {
	var aipObj = this.obj;
	aipObj.ShowFullScreen = 1;
}

AIP.prototype.setTrackRevision = function(e) {
	var aipObj = this.obj;
	if (e.value == '1')
		aipObj.TrackRevisions = 1;
	else
		aipObj.TrackRevisions = 0;

}

AIP.prototype.setShowRevision = function(e) {
	var aipObj = this.obj;
	if (e.value == '1')
		aipObj.ShowRevisions = 1;
	else
		aipObj.ShowRevisions = 0;
}
