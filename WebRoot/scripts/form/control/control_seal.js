	...
	//获取签章和批注的个数 
 	...
 	function signAndNotation(flowdefid,tempid){
 		...
 		$.ajax({
 	        type:'post',
 	        url:basePath+'/penaltyPerform/ajaxNonationNumber.action?1=1&flowdefid='+flowdefid+'&tempid='+tempid,
 	        async:false,
 	...
 	function noShow_signAndNotation(flowdefid,tempid){
 		...
 		$.ajax({
 	        type:'post',
 	        url:basePath+'/penaltyPerform/ajaxNoShowNonationNumber.action?1=1&flowdefid='+flowdefid+'&tempid='+tempid,
 	        async:false,
 	        success:function (text){
 	        	if(text){
 	        		signnotation = text.replace(/"/g ,"");
	...
	function doSubmitFlowAfterCheckSeal(type,number,resid,operateType,flowdefid,tempid){
		...
		var flowdraftid = '';
		var flowdraftidObj = mini.get("flowdraftid");
		if(flowdraftidObj != undefined) flowdraftid = flowdraftidObj.getValue();
		
		//如果为减刑假释审批表的审批
		var flag = -1;
		if(flowdefid == 'other_zfjyjxjssp'){
			//判断 如果 jyconfig:chengpibiao=0 那么就受 呈批表的控制
			var chengpibiao = mini.get("chengpibiao");
			if(chengpibiao != undefined){
		        chengpibiao = chengpibiao.getValue();
		    }
		    //减刑假释审核表提交时，要求先做好呈批表   jyconfig:chengpibiao=0 那么就受 呈批表的控制
		    if(chengpibiao == 0){
	...
	function updateProcessOfSignature(flowdraftid,operateType,flowdefid,tempid){
		...
		$.ajax({
	         url: basePath+"/penaltyPerform/ajaxsaveSignatureForAip.json", 
